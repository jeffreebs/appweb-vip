...
// src/pages/client/BookingScreen.jsx
...
// Importa el CSS nuevo y mejorado
...
const BookingScreen = () => {
  ...
  return (
    <div className="booking-background" style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url(${fondoBarberia})` }}>
      <div className="booking-container">
        <header className="booking-header">
          <h1>{isEditMode ? 'Editar Cita' : 'Agendar Nueva Cita'}</h1>
          <Link to={isEditMode ? '/my-appointments' : '/'} className="back-link">Volver</Link>
        </header>

        {!isEditMode && (
          <div className="step-indicator">
            <div className={`step-item ${step >= 1 ? 'active' : ''}`}><div className="step-circle">1</div><span>Servicio</span></div>
            <div className={`step-item ${step >= 2 ? 'active' : ''}`}><div className="step-circle">2</div><span>Barbero</span></div>
            <div className={`step-item ${step >= 3 ? 'active' : ''}`}><div className="step-circle">3</div><span>Fecha y Hora</span></div>
          </div>
        )}
              ...
              <div className="selection-grid">
                {services.map(s => (
                  <div key={s.id} className={`selection-card ${selectedService?.id === s.id ? 'selected' : ''}`} onClick={() => handleSelectService(s)}>
                    ...
                    <p className="card-details">₡{s.price.toLocaleString('es-CR')} - {s.duration} min</p>
                    {s.description && <p className="card-description">{s.description}</p>}
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="selection-grid">
                {barbers.map(b => (
                  <div key={b.id} className={`selection-card ${selectedBarber?.id === b.id ? 'selected' : ''}`} onClick={() => handleSelectBarber(b)}>
                    ...
                    <p className="card-details">{b.specialty}</p>
                  </div>
                ))}
              </div>
              <div className="action-buttons">
                <button onClick={() => setStep(1)} className="nav-btn">Volver a Servicios</button>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="date-time-picker">
                <div className="calendar-container">
                  <Calendar onChange={setSelectedDate} value={selectedDate} tileDisabled={tileDisabled} className="custom-calendar"/>
                </div>
                <div className="time-slots-container">
                  <h3>Horarios Disponibles</h3>
                  <div className="time-slots-grid">
                    {isEditMode && !availableTimeSlots.includes(selectedTime) && selectedTime && (
                      <div className="time-slot selected">{selectedTime} (Actual)</div>
                    )}
                    ...
                    {availableTimeSlots.length === 0 && !isEditMode && <p className="no-slots-message">No hay horarios disponibles.</p>}
                  </div>
                </div>
              </div>
              ...
              {selectedTime && (
                <div className="confirmation-summary">
                  ...
                  <p><strong>Fecha:</strong> {selectedDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'long' })}</p>
                  <p><strong>Hora:</strong> {selectedTime}</p>
                </div>
              )}

              <div className="action-buttons">
                <button onClick={() => isEditMode ? navigate('/my-appointments') : setStep(2)} className="nav-btn">
